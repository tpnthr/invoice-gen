...
import { Plus, Trash2, Receipt, Building, User, Package, CreditCard, Info } from "lucide-react";
            ...
            {data.items.map((item, index) => (
              <div key={index} className="bg-accent/30 p-4 rounded border border-border">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <Label>CN/PKWiU</Label>
                    <Input
                      value={item.cn_pkwiu || ""}
                      onChange={(e) => updateField(`items[${index}].cn_pkwiu`, e.target.value)}
                      placeholder="95.11.10.0"
                      className="font-mono"
                      data-testid={`input-item-pkwiu-${index}`}
                    />
                  </div>
                  <div>
                    <Label>Ilość</Label>
